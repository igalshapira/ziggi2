...
(function($) {
   ...
   $.widget("ui.weekCalendar", {
      ...
      today : function() {
         this._clearCalendar();
         this._loadCalEvents(this.options.date);
	 $(".wc-prev").removeAttr("disabled");
	 $(".wc-next").removeAttr("disabled");
      },
	 ...
	 if (this.options.minDate && newDate < this.options.minDate)
	 {
	     ...
	     $(".wc-prev").attr("disabled", "disabled");
	 }
	 $(".wc-next").removeAttr("disabled");
	 /* Don't allow before certain date */
         this._clearCalendar();
	 ...
	 {
	     newDate = this.options.maxDate;
	     $(".wc-next").attr("disabled", "disabled");
	 }
         this._clearCalendar();
         this._loadCalEvents(newDate);
	 ...
	 $(".wc-prev").removeAttr("disabled");
	  ...
	  return this._dateFirstDayOfWeek(date);
      },

      timeslotHeight : function() {
	  return this.options.timeslotHeight;
      },

      getTimeFromEvent : function(event) {
           var $target = $(event.target);
           var $weekDay = $target.closest(".wc-day-column-inner");
	   var columnOffset = $target.offset().top;
	   var top = event.pageY - columnOffset;
           return this._getEventDurationFromPositionedEventElement($weekDay, null, top);
      },
      ...
      _setupEventDelegation : function() {
         ...
         this.element.unbind("click").click(function(event) {
            ...
            if ($target.hasClass("wc-cal-event")) {
               return options.eventClick($target.data("calEvent"), $target, event);
            } else if ($target.parent().hasClass("wc-cal-event")) {
               return options.eventClick($target.parent().data("calEvent"), $target.parent(), event);
            } else if ($target.parent().parent().hasClass("wc-cal-event")) {
	    /* Ziggi: Allow trash button on wc-time */
               return options.eventClick($target.parent().parent().data("calEvent"), $target, event);
            }
	    ...
	    if ($target.parent().hasClass("wc-cal-event")) {
            ...
            if ($target.parent().hasClass("wc-cal-event")) {
               if ($target.parent().data("sizing")) return;
      ...
      _renderCalendar : function() {
         ...
         var $calendarContainer, calendarNavHtml, calendarHeaderHtml, calendarBodyHtml, $weekDayColumns, $buttonsContainer;
         var self = this;
         var options = this.options;
         ...
         if (options.buttons) {
            calendarNavHtml = "<div class=\"wc-nav\">\
                    <button title=\"" + options.buttonTitle.lastWeek + "\" class=\"wc-prev\">" + options.buttonText.lastWeek + "</button>\
                    <button title=\"" + options.buttonTitle.today + "\"class=\"wc-today\">" + options.buttonText.today + "</button>\
                    <button title=\"" + options.buttonTitle.nextWeek + "\"class=\"wc-next\">" + options.buttonText.nextWeek + "</button>\
                    </div>";

	    if (options.buttonsContainer)
	    {
		$(options.buttonsContainer).html(calendarNavHtml);
		...
		$buttonsContainer = $buttonsContainer;
            ...
            $buttonsContainer.find(".wc-nav .wc-today").click(function() {
               self.element.weekCalendar("today");
               return false;
            });

            $buttonsContainer.find(".wc-nav .wc-prev").click(function() {
               self.element.weekCalendar("prevWeek");
               return false;
            });

            $buttonsContainer.find(".wc-nav .wc-next").click(function() {
	    ...
	    if (options.is_monthly) {
               calendarBodyHtml += "<div class=\"wc-time-header-cell\">" + self._weekForIndex(i) + "</div>";
	    }
	    else  if (options.use24Hour) {
      ...
      _setupEventCreationForWeekDay : function($weekDay) {
         var self = this;
         var options = this.options;
         $weekDay.mousedown(function(event, force) {
	     if (!force) return;
            var $target = $(event.target);
	     ...
	     if (!force) return;
            ...
            if ($(event.target).hasClass("wc-day-column-inner")) {
		 return options.weeklyClick(event);
	     }
	 });
      },
	 ...
	 var name;
	    ...
	    name = dayName;
	    if (!options.hideDate)
	    name += "<br/>" + self._formatDate(currentDay, options.dateFormat);
            $(this).html(name);
	 ...
	 if (options.drawCurrentTimeLine) {
	     var d = new Date();
	     if (!self.options.businessHours.limitDisplay ||
		 (d.getHours() > self.options.businessHours.start &&
		 d.getHours() < self.options.businessHours.end))
	     {
		 eventsToRender.push({
			 "id" : "drawCurrentTimeLine",
		         "className": "wc-cal-now",
			 "start" : d,
			 "end" : d,
			 "readOnly" : true
			 });
         ...
         if (!eventsToRender.length || (options.drawCurrentTimeLine && eventsToRender.length == 1)) {
            options.noEvents();
         }

      },

      _renderFullDayEvent: function (calEvent, $weekDay) {
	   /* Do it the stupid way, but it works */ 
	   var c = /day-([0-9])/.exec($weekDay.parent().attr("class"));
	   var $header = $(".wc-day-column-header.wc-day-" + c[1]);
	   $header.append($("<div/>").addClass(calEvent['className']).html(calEvent.title));
       },
      /*
	 ...
	 /* Events at midnight are assumed to be full day events */
         if (calEvent.start.getTime() == calEvent.end.getTime() &&
		 calEvent.start.getHours() == 0) {
	     return self._renderFullDayEvent(calEvent, $weekDay);
	 }
         var eventClass, eventHtml, $calEvent, $modifiedEvent;

         eventClass = calEvent.id ? "wc-cal-event" : "wc-cal-event wc-new-cal-event";
	 if (calEvent.className)
	     eventClass += " " + calEvent.className;
      ...
      _groupOverlappingEventElements : function($weekDay) {
         ...
         $.each(sortedEvents, function() {
            $curEvent = $(this);
	    if (($curEvent).data("calEvent").id == "drawCurrentTimeLine")
		return;
      ...
      _positionEvent : function($weekDay, $calEvent) {
         ...
         $calEvent.css({top: pxTop, height: Math.max(pxHeight, 1)});
      },
	 ...
	 else
	 {
	     ...
	     $calEvent.find(".wc-title").html(calEvent.content);
	 }
	 $calEvent.data("calEvent", calEvent);
      ...
      _weekForIndex : function(index) {
	  var HOUR = 1000 * 60 * 60;
	  var WEEK = HOUR * 24 * 7;
	  var date = this.options.date;
	  var d = new Date();
	  d.setTime(date.getTime() + WEEK*index);
	  return this._formatDate(d, this.options.dateFormat);
      },
   ...
   $.weekCalendar = function() {
      return {
	     ...
	     /* Return date of Sunday in this week */
	     var midnightCurrentDate = new Date(date.getFullYear(), date.getMonth(), date.getDate());
	     var millisToSubtract = midnightCurrentDate.getDay() * 86400000;
	     var d = new Date(midnightCurrentDate.getTime() - millisToSubtract);
	     d.setTime(d.getTime() - d.getTimezoneOffset() * MINUTE);
	     return d;
	 },
         getSaturday : function(date) {
	     /* Return date of Sunday in this week */
	     var d = this.getSunday(date);
	     d.setTime(d.getTime() + 6 * 24 * 60 * MINUTE);
	     return d;
	 }
      };
   }();
